...
#### Стандартные критерии приемки после настройки окружения

**Важно**: После завершения Phase A (Foundations) все задачи должны включать следующие обязательные критерии приемки в дополнение к специфичным для задачи:

1. **PHPStan**: Отсутствие ошибок статического анализа
   - PHPStan level 9 выполняется без ошибок
   - Команда: `./dev phpstan` или `vendor/bin/phpstan analyse` проходит успешно
   - Критерий: `✅ PHPStan level 9 без ошибок`

2. **Линтер**: Код соответствует стандартам форматирования
   - PHP-CS-Fixer проверяет и исправляет форматирование кода
   - Команда: `./dev cs-fix` или `vendor/bin/php-cs-fixer fix --dry-run --diff` не находит нарушений
   - Критерий: `✅ PHP-CS-Fixer не находит файлов для исправления` или `✅ Все файлы отформатированы согласно PSR-12`

3. **Тесты**: Все тесты выполняются успешно
   - Все существующие тесты проходят
   - Команда: `./dev phpunit` или `vendor/bin/phpunit` завершается с кодом возврата 0
   - Критерий: `✅ Все тесты проходят` или `✅ PHPUnit тесты выполняются без ошибок`

**Применение**:
- Эти критерии **обязательны** для всех задач, которые зависят от задач Phase A (особенно A4: Настроить инструменты качества кода)
- В поле **Acceptance Criteria** таблицы задач эти критерии должны быть указаны явно или через ссылку на стандартные критерии
- При проверке завершения задачи все три критерия должны быть выполнены

**Пример**:
```markdown
| B1 | Создать модели данных | High | 2 | A5 | PHP, PHPUnit | Модели созданы, покрыты тестами (TDD: Red-Green-Refactor), PHPStan level 9, PHP-CS-Fixer, все тесты проходят | Open |
```
...
|---------|-------------|----------|--------|--------------|-------|---------------------|
| T1 | Подзадача 1 | High | 1d | None | PHPUnit | Тесты проходят, PHPStan level 9 без ошибок, PHP-CS-Fixer, все тесты проходят |
...
- [ ] Для задач после Phase A включены стандартные критерии приемки (PHPStan, PHP-CS-Fixer, тесты)
- [ ] Edge cases описаны
